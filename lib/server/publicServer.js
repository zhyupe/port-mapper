var net = require('net');
var logger = null;
var auth = require('./publicAuth');

var PublicServer = function (connectServer, publicAuth) {
    this.publicAuth    = !!publicAuth;
    if (this.publicAuth) {
        auth.setConnectServer(connectServer);
    }
...
publicServer.getHandler = function () {
    ...
    return function (sock) {
        ...
        if (self.connectServer.status != self.connectServer.STATUS.available) {
            logger.warn('[publicServer.handler][' + sock.id + '] Connect server is unavailable. Closed.');
            sock.end();
            return;
        }
        ...
        if (self.publicAuth && !auth.check(sock.remoteAddress)) {
            auth.handle(sock);
        } else {
...
module.exports = function (_logger, publicPort, connectServer, publicAuth) {
    logger = _logger;

    var _publicServer = new PublicServer(connectServer, publicAuth);
